...
def check_proxy_usage(ipv6, port, user, password, expected_ipv6):
    ...
    try:
        cmd = f'curl --proxy http://{proxy_user}:{proxy_pass}@{proxy_ip}:{proxy_port} --connect-timeout 5 http://{user}:{password}@[{ipv6}]:{port}/https://api64.ipify.org?format=json'
        ...
        if result.returncode == 0:
            ...
            try:
                ...
                # Chỉ đánh dấu là đã sử dụng nếu IP trả về khác real_ip của proxy trung gian
                is_used = ip != real_ip
                return is_used, ip
            except ValueError:
...
def auto_check_proxies():
    while True:
        try:
            conn = sqlite3.connect('proxies.db')
            c = conn.cursor()
            c.execute("SELECT ipv6, port, user, password, ipv6 AS expected_ipv6 FROM proxies")
            proxies = c.fetchall()
...
def message_handler(update: Update, context: CallbackContext):
    ...
    if state == 'prefix':
        ...
        try:
            ...
            ipv6, port, user, password = proxy.split(':')
            days = int(days)
        ...
        try:
            ipv6, port, user, password = text.split(':')
            conn = sqlite3.connect('proxies.db')
            c = conn.cursor()
