...
def check_proxy_usage(ipv4, port, user, password, expected_ipv6):
    ...
    try:
        # Sử dụng IPv6-only endpoint để đảm bảo chỉ trả về IPv6
        ...
        if result.returncode == 0:
            ...
            try:
                ...
                return False, ip  # Trả về False nếu IP không phải IPv6
        else:
...
def create_proxy(ipv4, ipv6_addresses, days):
    try:
        ...
        proxies = []
        # Đảm bảo file squid.conf có cấu hình cơ bản, ưu tiên IPv6
        squid_conf_base = """
...
# Vô hiệu hóa IPv4 cho outgoing traffic
tcp_outgoing_addr 0.0.0.0/0
"""
