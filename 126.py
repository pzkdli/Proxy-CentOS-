...
def ensure_squid_running():
    try:
        result = subprocess.run(['systemctl', 'is-active', 'squid'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
        if result.stdout.strip() != 'active':
            logger.info("Squid chưa chạy, đang khởi động...")
            subprocess.run(['systemctl', 'start', 'squid'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            time.sleep(2)  # Đợi Squid khởi động
            result = subprocess.run(['systemctl', 'is-active', 'squid'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            if result.stdout.strip() != 'active':
