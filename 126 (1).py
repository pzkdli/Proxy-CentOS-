...
def ensure_squid_running():
    try:
        ...
        if result.stdout.strip() != 'active':
            ...
            if result.returncode != 0:
                logger.warning(f"Không thể khởi động Squid bằng systemctl: {result.stderr}. Thử chạy trực tiếp...")
                # Chạy Squid trong foreground nếu systemctl không khả dụng
                subprocess.run(['squid', '-z'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
                subprocess.Popen(['squid', '-N', '-d1'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            time.sleep(3)  # Đợi Squid khởi động
            # Kiểm tra lại trạng thái
            result = subprocess.run(['systemctl', 'is-active', 'squid'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, check=False)
            if result.stdout.strip() != 'active':
                # Kiểm tra xem Squid có chạy không bằng ps
                result = subprocess.run(['ps', 'aux'], stdout=subprocess.PIPE, universal_newlines=True, check=False)
                if 'squid' not in result.stdout:
...
def check_proxy_usage(ipv4, port, user, password, expected_ipv6):
    ...
    try:
        cmd = f'curl --proxy http://{proxy_user}:{proxy_pass}@{proxy_ip}:{proxy_port} --connect-timeout 5 http://{user}:{password}@{ipv4}:{port}/https://api64.ipify.org?format=json'
...
"""
        ...
        if result.returncode != 0:
            logger.warning(f"Không thể restart Squid bằng systemctl: {result.stderr}. Thử chạy trực tiếp...")
            # Dừng Squid hiện tại nếu có
            subprocess.run(['squid', '-k', 'shutdown'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            time.sleep(2)
            # Xóa file PID
            if os.path.exists('/var/run/squid.pid'):
                os.remove('/var/run/squid.pid')
            # Chạy Squid trong foreground
            subprocess.Popen(['squid', '-N', '-d1'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
...
def button(update: Update, context: CallbackContext):
    ...
    if query.data == 'new':
        ...
        proxy_settings = get_proxy_settings()
        if not proxy_settings:
            query.message.reply_text("Chưa cài đặt proxy trung gian! Vui lòng dùng lệnh /set_proxy để cài đặt.")
            return
