...
def check_ipv6_support():
    try:
        # Kiểm tra xem có IPv6 được gán trên eth0
        result = subprocess.run(['ip', '-6', 'addr', 'show', 'dev', 'eth0'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, timeout=5)
        if 'inet6' not in result.stdout:
            logger.error("Không có địa chỉ IPv6 nào được gán trên eth0")
            return False
...
def check_proxy_usage(ipv4, port, user, password, expected_ipv6):
    ...
    try:
        ...
        cmd = f'curl --proxy http://{proxy_user}:{proxy_pass}@{proxy_ip}:{proxy_port} --connect-timeout 5 --ipv6 http://{user}:{password}@{ipv4}:{port}/https://api64.ipify.org?format=json'
        ...
        if result.returncode == 0:
            ...
            try:
                ...
                # Proxy được coi là đã sử dụng nếu IP trả về là IPv6 và khác real_ip
                is_used = (ip != real_ip and ip == expected_ipv6)
                if ip != expected_ipv6:
    ...
    while True:
        try:
            ...
            for proxy in proxies:
                ipv4, port, user, password, expected_ipv6 = proxy
                is_used, returned_ip = check_proxy_usage(ipv4, port, user, password, expected_ipv6)
...
"""
        ...
        logger.info(f"Đã tạo {len(proxies)} proxy với IPv4 {ipv4}")
...
def button(update: Update, context: CallbackContext):
    ...
    if query.data == 'new':
        ...
        query.message.reply_text("Nhập proxy và số ngày gia hạn (định dạng: ipv4:port:user:pass số_ngày):")
        context.user_data['state'] = 'giahan'
    elif query.data == 'xoa_le':
        query.message.reply_text("Nhập proxy cần xóa (định dạng: ipv4:port:user:pass):")
...
def message_handler(update: Update, context: CallbackContext):
    ...
    if state == 'prefix':
        ...
        try:
            ...
            update.message.reply_text("Định dạng không hợp lệ! Vui lòng nhập: ipv4:port:user:pass số_ngày")
    elif state == 'xoa_le':
        if not check_required_settings(context):
            update.message.reply_text("Vui lòng hoàn thành các bước sau trước:\n1. Nhập prefix IPv6 và IPv4 bằng /start\n2. Cài đặt proxy trung gian bằng /set_proxy")
            return
